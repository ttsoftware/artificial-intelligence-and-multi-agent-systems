...
public class HTNPlanner {
    ...
    public HTNPlan plan() {
        ...
        int gotoDistance = LevelService.getInstance().manhattenDistance(agent, box);

        Position boxPosition = LevelService.getInstance().getLevel().getBoardObjectPositions().get(box.getLabel());

        actions.add(new GotoAction(gotoDistance, boxPosition.getRow(), boxPosition.getColumn()));

        // Move this box to the goal - estimate distance
        int moveDistance = LevelService.getInstance().manhattenDistance(box, goal);

        actions.add(new MoveBoxAction(moveDistance, goal.getRow(), goal.getColumn()));
